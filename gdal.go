...
func OSRSetProjSearchPaths(projpaths []string) {
	length := len(projpaths)
	papszPaths := make([]*C.char, length+1)
	for i := 0; i < length; i++ {
		papszPaths[i] = C.CString(projpaths[i])
		defer C.free(unsafe.Pointer(papszPaths[i]))
	}
	C.OSRSetPROJSearchPaths((**C.char)(unsafe.Pointer(&papszPaths[0])))
}
